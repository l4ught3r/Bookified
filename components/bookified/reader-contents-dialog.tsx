...
import { FocusTrapScope } from "@/components/ui/focus-trap";
...
const TAB_ORDER: ContentsTab[] = ["toc", "bookmarks"];
...
export function ReaderContentsDialog({
  ...
  const tocTabId = `${tabBaseId}-tab-toc`;
  const bookmarksTabId = `${tabBaseId}-tab-bookmarks`;
  const tocPanelId = `${tabBaseId}-panel-toc`;
  const bookmarksPanelId = `${tabBaseId}-panel-bookmarks`;
  ...
  return createPortal(
    <>
      ...
      <FocusTrapScope initialFocusRef={closeRef}>
        <aside
          role="dialog"
          aria-modal="true"
        ...
        >
          ...
          >
            {(
              [
                { id: "toc" as const, label: t("toc"), tabId: tocTabId, panelId: tocPanelId },
                {
                  id: "bookmarks" as const,
                  label: t("bookmarksTab"),
                  tabId: bookmarksTabId,
                  panelId: bookmarksPanelId,
                },
              ] as const
            ).map((tab) => (
              <button
                    ...
                    <li key={chapter.order}>
                      ...
                      >
                        ...
                        >
                          {chapter.title}
                          {isCurrent ? (
                            <span className="sr-only"> — {t("currentChapter")}</span>
                          ) : null}
                        </span>
                        {progressLabel != null ? (
